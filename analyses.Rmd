...
pacman::p_load(adehabitatHR, adehabitatLT, brglm, corrplot, data.table,
               ...
               plotrix, rstudioapi, raster, readxl, rgdal, RODBC,
...
```{r, include=FALSE}
jpeg(file="figures/distance across individuals.jpeg", 
...
random_effects <- model_data %>%
  dplyr::select(night_12hr, day, min_temp, max_temp, precip, moon) %>%
...
```{r, include=FALSE}
jpeg(file="figures/ranges per cohort.jpeg", 
...
```{r, include=FALSE}
jpeg(file="figures/nvis habitat barplot and map.jpeg", 
...
```{r, include=FALSE}
jpeg(file="figures/nvis habitat frequency.jpeg", 
...
lidar_os_plot <- ggplot(data=lidar_os, aes(x=type, y=raster_units)) +
  ...
  annotate(geom="text", x=2, y=72, label="a)", fontface="bold", size=6)
...
lidar_us_plot <- ggplot(data=lidar_us, 
  ...
  annotate(geom="text", x=2, y=39, label="b)", fontface="bold", size=6)
...
lidar_as_plot <- ggplot(data=lidar_as, aes(x=type, y=raster_units)) +
  ...
  annotate(geom="text", x=2, y=62, label="c)", fontface="bold", size=6)
...
```{r, include=FALSE}
jpeg(file="figures/lidar habitat use.jpeg", 
...
```{r}
# Summary statistics for Pearson and period
interact %>% 
  group_by(period) %>%
  summarise(cr_mean=mean(cr), cr_se=std.error(cr)) 

# Summary statistics for Pearson and morph
interact %>% 
  group_by(morph) %>%
  summarise(cr_mean=mean(cr), cr_se=std.error(cr)) 
```
...
pear_period_plot <- ggplot(data=interact, aes(x=period, y=as.numeric(cr))) +
  ...
  annotate(geom="text", x=0.75, y=0.8, label="a)", fontface="bold", size=6)
...
pear_morph_plot <- ggplot(data=interact, aes(x=morph, y=as.numeric(cr))) +
  ...
  scale_fill_manual(values = viridis(2, begin=0.9, end=0.1)) +
  xlab("Morph") + ylab("") +
  annotate(geom="text", x=0.7, y=0.8, label="b)", fontface="bold", size=6)
...
coden_plot <- ggplot(data=sharing_plot, 
  ...
  annotate(geom="text", x=0.65, y=0.95, label="c)", fontface="bold", size=6)  
...
```{r, include=FALSE}
jpeg(file="figures/interaction by fixed effects.jpeg", 
     width=4500, height=1500, units="px", res=500)
print(interact_plot)
dev.off()
```
...
# Nocturnal activity
mod <- lmer(percentage ~ cohort*period + morph + (1|name), data=woodland)
anova(mod) #F-value
car::Anova(mod) #chisq + p-value, 
summary(mod) #random effects

mod <- lmer(percentage ~ cohort*period + morph + (1|name), data=forest)
anova(mod) #F-value
car::Anova(mod) #chisq + p-value, 
summary(mod) #random effects

mod <- lmer(percentage ~ cohort*period + morph + (1|name), data=regrowth)
anova(mod) #F-value
car::Anova(mod) #chisq + p-value, 
summary(mod) #random effects

mod <- lmer(percentage ~ cohort*period + morph + (1|name), data=grassland)
anova(mod) #F-value
car::Anova(mod) #chisq + p-value, 
summary(mod) #random effects

mod <- lmer(overstory ~ cohort*period + morph + (1|name), data=noc_lidar)
anova(mod) #F-value
car::Anova(mod) #chisq + p-value, 
summary(mod) #random effects

mod <- lmer(understory ~ cohort*period + morph + (1|name), data=noc_lidar)
anova(mod) #F-value
car::Anova(mod) #chisq + p-value, 
summary(mod) #random effects

mod <- lmer(aspect ~ cohort*period + morph + (1|name), data=noc_lidar)
anova(mod) #F-value
car::Anova(mod) #chisq + p-value, 
summary(mod) #random effects
...
mod <- lmer(overstory ~ cohort*period + morph + (1|name), data=diu_lidar)
anova(mod) #F-value
car::Anova(mod) #chisq + p-value, 
summary(mod) #random effects

mod <- lmer(understory ~ cohort*period + morph + (1|name), data=diu_lidar)
anova(mod) #F-value
car::Anova(mod) #chisq + p-value, 
summary(mod) #random effects

mod <- lmer(aspect ~ cohort*period + morph + (1|name), data=diu_lidar)
anova(mod) #F-value
car::Anova(mod) #chisq + p-value, 
summary(mod) #random effects
