...
This [R project](https://support.posit.co/hc/en-us/articles/200526207-Using-RStudio-Projects) presents the datasets and workflows used for Wilson *et al.* (2025a) "*Everybody needs good neighbours: movement, habitat use, and conspecific association in a reintroduced mesopredator*", published June 2024 in [Biodiveristy and Conservation](https://doi.org/10.1007/s10531-024-02884-z). For more information, see the [GitHub repo](https://github.com/coexistence-conservation-lab/**repo**) README.

We used GPS collars to quantify: 

 - **Distances travelled**
 - **Home** and **core ranges**, 
 - **Nocturnal (activity)** and **diurnal denning** habitat use and selection, and 
 - **Conspecific associations**. 

We compared these measures across three distinct periods:
...
When you [knit this document 🧶](https://rmarkdown.rstudio.com/authoring_quick_tour.html) (e.g., render to HTML), you can add **chunk options** to control what is shown in the output (e.g., `{r, message = FALSE, results = 'hide', warning = FALSE}`). To avoid long chunk headers like the one above, use these custom options:  
...
pacman::p_load(
  ...
  # Data wrangling
  data.table, dplyr, grid, gridExtra, gtools, 
  janitor, lubridate, tidyr, tidyverse,
  
  # Mapping
  ggspatial, ozmaps, raster, sf, sp, 
  # rgdal is now deprecated
  
  # Plotting
  ggplot2, ggpmisc, ggpubr, plotrix, scales, viridis, 
  
  # Spatial analyses
  adehabitatHR, adehabitatLT, MuMIn, wildlifeDI, 
  
  # Statistical modelling
  AICcmodavg, beepr, brglm, corrplot, here, lme4)
```
...
```{r}
# Define MFGO centre coordinates
mfgo_lat <- -35.184396
mfgo_lon <- 149.171633
...
```{r}
# Define CRS for WGS (lat/lon) and UTM (eastings/northings)
wgs <- "EPSG:4326"
utm <- "EPSG:32755"
```

```{r}
# Define colours for areas of interest
aoi_cols <- c(
  "act" = "#fffe92",
  "nsw" = "#289fc4" ,
  "tas" = "#29a145",
...
Load project-specific functions from `functions.R`. 
...
wea <- read_excel(raw_data, sheet = "weather") %>% 
  clean_names() %>%
...
coords <- left_join(gps, wea, by = "date") %>%
  ...
  mutate(period         = 
           ifelse(night_12hr >= 3  & night_12hr <= 21, 'Baseline', 
           ifelse(night_12hr >= 22 & night_12hr <= 32, 'Release',
           ifelse(night_12hr >= 33 & night_12hr <= 52, 'Settlement',
                  NA)))) %>%
...
```{r}
# Read in MFGO boundary and assign projection
mfgo_poly <- read_gpkg("mfgo_boundaries.gpkg") %>%
  st_transform(wgs)

# Subset to MFGO (GS + MFWS)
mfgo <- mfgo_poly %>%
  filter(site %in% c(
...
# Subset to MFWS
mfws <- mfgo_poly %>%
  filter(site == "Mulligans Flat Woodland Sanctuary")

# Filter fixes to those inside MFWS
coords_filt <- st_filter(coords_sp, mfws) 
```

```{r}
# Fetch Australian state boundaries 
aus <- ozmap_states |>
  st_transform(st_crs(wgs)) |>
...
# Subset to AOIs
act  <- aus |> filter(NAME == "act")
nsw  <- aus |> filter(NAME == "nsw")
seat <- aus |> filter(NAME %in% c("act", "nsw", "tas", "vic"))
...
Here we use the [National Vegetation Information System (v6.0, now superseded by v7.0)](https://fed.dcceew.gov.au/datasets/be9930d6de354ace93fd1aa5d34a71de/about) major extant vegetation groups polygon (adapted for MFWS, see **Habitat use** section) to classify points as **inside** or **outside** the sanctuary.

### Convert GDB to GPKG

```{r, eval = FALSE}
# Check available layers in the NVIS vector geodatabase 🐢
st_layers(dsn = file.path(geo_in, "nvis/NVIS_V7_0_VECTOR_STATE_FILES_EXT.gdb"))

# Write ACT layer to .gpkg 🐢🐢🐢
sf::gdal_utils(
  util        = "vectortranslate",
  source      = file.path(geo_in, "nvis/NVIS_V7_0_VECTOR_STATE_FILES_EXT.gdb"),
  destination = file.path(geo_in, "act_nvis.gpkg"),
  options     = c("-f", "GPKG", "-overwrite",
                  "-sql", "SELECT * FROM NVIS7_0_AUST_EXT_ACT")); beep(sound)

# Read ACT NVIS layer 🐢
act_nvis <- st_read(file.path(geo_in, "act_nvis.gpkg")) %>%
  st_transform(wgs) %>%
  st_make_valid(); beep(1)

# Replace raw GDAL output with projected and valid vector
st_write(
  act_nvis,
  dsn    = file.path(geo_in, "act_nvis.gpkg"),
  layer  = "act_nvis",
  driver = "GPKG", 
  append = FALSE, 
  quiet  = TRUE)
...
Fetch MVG and MVS lookup table.

```{r, eval = FALSE}
# Read ACT NVIS attributes only (no geometry) to get present MVS codes
act_nvis_att <- st_read(
  file.path(geo_in, "act_nvis.gpkg")) |>
  st_drop_geometry()

# Read the flat lookup table containing MVG and MVS codes and names
nvis_lu <- st_read(
  dsn   = file.path(geo_in, "nvis/NVIS_V7_0_VECTOR_STATE_FILES_EXT.gdb"),
  layer = "NVIS7_0_LUT_AUST_FLAT"); beep(sound)

# Extract all NVIS_IDs present in ACT spatial layer
act_ids <- act_nvis_att |>
  select(starts_with("NVISDSC")) |>
  unlist() |>
  unique() |>
  na.omit()

# Filter lookup to only records present in ACT, keeping MVG/MVS columns
act_nvis_lu <- nvis_lu |>
  filter(NVIS_ID %in% act_ids) |>
  select(NVIS_ID, L5_ASSOCIATION, MVG_NUMBER, MVG_NAME, MVS_NUMBER, MVS_NAME)
```

### Clip to MFGO and MFWS

```{r}
# Read ACT NVIS layer 🐢
act_nvis <- st_read(file.path(geo_in, "act_nvis.gpkg"), quiet = TRUE) %>%
  st_transform(wgs) %>%
  st_make_valid(); beep(1)

# Clip NVIS to MFGO boundary and write to .gpkg 🐢
mfgo_nvis <- act_nvis |> st_intersection(mfgo)
st_write(
  mfgo_nvis,
  dsn    = file.path(geo_in, "mfgo_nvis.gpkg"),
  layer  = "mfgo_nvis",
  driver = "GPKG", 
  append = FALSE, 
  quiet  = TRUE)

# Clip NVIS to MFWS boundary and write to .gpkg 🐢
mfws_nvis <- act_nvis |> st_intersection(mfws)
st_write(
  mfws_nvis,
  dsn    = file.path(geo_in, "mfws_nvis.gpkg"),
  layer  = "mfws_nvis",
  driver = "GPKG", 
  append = FALSE, 
  quiet  = TRUE)
...
```{r, message = FALSE}
nvis_dat <- read_excel(raw_data, sheet = "nvis") %>%
  clean_names() %>%
  
  # Subset to relevant version
  filter(nvis_version == 6) %>%

  # Select and rename columns
  select(
    mvg_n        = mvg_number, 
    nvis_habitat = mvg_name, 
    mfws_habitat = mfgo_name)

# Annotate polygon data to GPS df
coords_annotated <- coords_sp %>% 
  
  mutate(mvg_n = as.numeric(st_intersects(coords_sp, mfws_nvis))) %>%
  st_drop_geometry() %>% 
  left_join(nvis_dat, by = "mvg_n") %>%
  
  mutate(fence = factor(
    ifelse(!is.na(mfws_habitat), "Inside", "Outside"), 
...
mfws <- readOGR(
  dsn = file.path(geo_in, "mfws_fence.shp"), verbose = FALSE) %>%
...
ggplot() + 
  ...
  scale_colour_manual(
    values = viridis(3, begin = 0.9, end = 0.1), 
    name   = "Fence") +
...
mod_cov <- glm(
  fence ~ duration + temp + dop + satellites, 
  family = binomial(link = logit), 
  data   = coords_annotated)

# Calculate AIC of each model
aictab(
  cand.set = list(mod_null, mod_cov), 
...
# Save plot to tiff
ggsave(
  filename = "output/Fig 0 (data curation).tiff", 
  covariates, 
  height = 125, 
  width = 150, 
  units = "mm")
...
for(i in quolls){
  ...
  night <- as.POSIXct(strptime(
    as.character(quolls$date_time_12hr),
...
distance_night <- distance_fix %>% #distance per night
  ...
  mutate(
    distance = as.numeric(distance/1000), #gives kilometres
    night_12hr = as.numeric(night_12hr)) %>% 
  
  group_by(
    cohort, period, morph, name, date_12hr, night_12hr) %>%
...
data <- distance_night %>% 
  na.omit() %>%
  mutate(date_12hr = as.POSIXct(strptime(
    date_12hr, format = "%Y-%m-%d", tz = "Australia/Sydney")))

# Read in environmental covariates
wea <- read_excel(raw_data, sheet = "environment") %>% 
  clean_names() %>%
...
round(coef(summary(lmer(
  distance_fix ~ morph + (period*cohort) + 
    (1|name) + (1|precip) + (1|min_temp) + (1|moon), 
  data = model_data))), 
  digits = 3)
...
# Print model statistics for fixes and random predictors
round(coef(summary(
  lmer(distance_night ~ morph + (period*cohort) +
         (1|name) + (1|precip) + (1|min_temp) + (1|moon), 
       data = model_data))), 
  digits = 3)
```
...
data <- read.csv("output/coordinates curated.csv") %>%
  mutate(night_date = as.POSIXct(strptime(
    date_time_12hr, format = "%Y-%m-%d")))

coords <- SpatialPointsDataFrame(
  coordinates(cbind(data$easting, data$northing)), data = data)
```
...
for (i in 1:length(quolls)) {
  for (j in 1:length(days)) {
    ...
    out        <- as.data.frame(x = cbind(
      home_range[["area"]], core_range[["area"]], 
      quolls[i], days[j])) %>% 
...
ranges_data <- data %>% 
  select(night_date, night_12hr, day_12hr) %>%
...
ranges_data <- ranges_data %>% 
  mutate(period = ifelse(night_12hr <= 19,                    'Baseline', 
                  ...
                  ifelse(night_12hr >= 29,                    'Settlement', 
                         NA)))) %>% 
...
home_range <- getverticeshr(kud, percent = 95) %>% 
  ...
  st_set_crs(st_crs(utm)) #set CRS to UTM
...
# Extract areas
ranges_study <- data.frame(left_join(
  st_drop_geometry(home_range),
...
for (i in seq_along(quolls)) { #errors for NULL coordinates are okay
  ...
  settlement <- coords %>% subset(
    name == quolls[i] & period == "Settlement", 
    select = name)
...
# Extract areas
ranges_period <- data.frame(left_join(
  st_drop_geometry(hr), 
  st_drop_geometry(cr), 
  by = c("id", "period"))) %>%
...
mod <- glm(
  home_range ~ cohort + period + morph, 
  data = ranges_model)
...
mod <- glm(
  core_range ~ cohort + period + morph, 
  data = ranges_model)
...
ranges_data <- read.csv("output/ranges per day.csv") %>%
  mutate(cohort = factor(
    cohort, levels = c("Resident", "Reinforcer"))) %>%
  
  left_join(random_effects, by = "night_12hr")

mod <- glm(
  home_range ~ night_12hr + cohort + morph, 
  data = ranges_data)
...
mod <- glm(
  core_range ~ night_12hr + cohort + morph, 
  data = ranges_data)
...
hr_lookup <- home_range %>% 
  ...
  mutate(cohort = ifelse(
    cohort == "Resident", "Residents", "Reinforcers")) %>%
...
hr_cr_map <- ggplot(hr_cr_lookup) +
  ...
  coord_sf(
    xlim = c(695750, 699505), 
    ylim = c(6104500, 6107675)) +
...
```{r, include = FALSE}
tiff(
  file = "output/Fig 3 (ranges map).tiff", 
  height = 1500, 
  width = 1700, 
  units = "px", 
  res = 300)
...
overlap <- data.frame(left_join(
  st_drop_geometry(home_overlap), 
  st_drop_geometry(core_overlap), 
  by = c("name", "name2"))) %>%
  
  rename(
    home_overlap = overlap.x, 
    core_overlap = overlap.y)
...
round(coef(summary(glm(
  home_overlap ~ cohort + morph, 
  data = overlap))),
  digits = 5)

round(coef(summary(glm(
  core_overlap ~ cohort + morph, 
  data = overlap))), 
  digits = 5)
```
...
tiff(
  file = "output/Fig 2a–d (distance and ranges).tiff", 
  height = 1600,
  width = 2150, 
  units = "px", 
  res = 300)
...
nvis_barplot <- ggplot(
  ...
  geom_text(aes(
    label = paste0(round(perc, digits = 1), "%"), 
    vjust = -0.5)) +
  ...
  theme(
    ...
    axis.text.y = element_text(
      angle = 0, vjust = 0.5, hjust = 0, color = "black"),
    axis.text.x = element_text(
      angle = 0, vjust = 1, hjust = 0.5, color = "black")) +
...
```{r, include = FALSE}
tiff(
  file = "output/Fig 1a–b (habitat type and map).tiff", 
  width = 3000, 
  height = 1250, 
  units = "px", 
  res = 300)
...
noc_lidar <- noc_sp %>%
  mutate(
    overstory = raster::extract(overstory, noc_sp, method = "simple"),
...
overstory_mf <- as.data.frame(rasterToPoints(mask(overstory, mfws))) %>%
  mutate(
    type         = "Available", 
    raster       = "Overstory", 
...
understory_mf <- as.data.frame(rasterToPoints(mask(understory, mfws))) %>%
  mutate(
    type         = "Available", 
    raster       = "Understory",
...
aspect_mf <- as.data.frame(rasterToPoints(mask(aspect, mfws))) %>%
  mutate(
    type         = "Available", 
    raster       = "Aspect",
...
woodland <- subset(
  noc_model, 
  mfws_habitat == "Eucalypt woodland")

round(coef(summary(lmer(
  percentage ~ cohort*period + morph + 
    (1|name), 
  data   = woodland))), 
  digits = 4)
...
```{r}
mod <- lmer(
  overstory ~ cohort*period + morph + (1|name), data = noc_lidar)
...
mod <- lmer(
  understory ~ cohort*period + morph + (1|name), data = noc_lidar)
...
mod <- lmer(
  aspect ~ cohort*period + morph + (1|name), data = noc_lidar)
...
data.frame(
  ...
  Mean = c(
    mean(noc_lidar$overstory)*100, 
    mean(noc_lidar$understory)*100,
    mean(noc_lidar$aspect)),
  
  SE = c(
    std.error(noc_lidar$overstory)*100, 
...
diu_adj <- diu_nvis %>%
  ...
  mutate(
    freq = n, 
    perc = (freq/413)*100) #adjusted by frequency of dens

diu_plot <- diu_adj %>% clean_names() %>%
  left_join(mfws_group, by = "mfws_habitat") %>%
  mutate(
    freq_adj     = freq/area, #adjusted by total area
...
diu_lidar <- diu_sp %>%
  mutate(
    overstory  = raster::extract(overstory,  diu_sp, method = "simple"),
...
```{r}
round(coef(summary(lmer(
  overstory ~ cohort*period + morph + 
    (1|name), 
  data   = diu_lidar))), 
  digits = 4)

round(coef(summary(lmer(
  understory ~ cohort*period + morph + 
    (1|name), 
  data   = diu_lidar))), 
  digits = 4)

round(coef(summary(lmer(
  aspect ~ cohort*period + morph + 
    (1|name), 
  data   = diu_lidar))), 
  digits = 4)
```
...
data.frame(
  ...
  Mean = c(
    mean(diu_lidar$overstory)*100, 
    mean(diu_lidar$understory)*100,
    mean(diu_lidar$aspect)),

  SE = c(
    std.error(diu_lidar$overstory)*100, 
...
noc_lidar_plot <- noc_lidar %>%
  ...
  mutate(
    'Overstory'  = overstory*100, 
    'Understory' = understory*100, 
    'Aspect'     = aspect) %>%
  ...
  select(quoll, cohort, period, morph, 
...
diu_lidar_plot <- diu_lidar %>%
  ...
  select(
    quoll, cohort, period, morph, day, raster, raster_units, type)

lidar_plot <- rbind(noc_lidar_plot, diu_lidar_plot) %>%
  
  mutate(type    = factor(
    type, levels = c("Nocturnal activity", "Diurnal denning"))) %>%
  select("type", "raster", "raster_units") %>%
  rbind(raster_mf)
```
...
data.frame(
  ...
  Mean = c(
    mean(avai_os$raster_units), 
    mean(avai_us$raster_units),
    mean(avai_as$raster_units)),
  
  SE = c(
    std.error(avai_os$raster_units), 
...
noc_nvis_plot <- ggplot(
  ...
  geom_text(aes(
    label = paste0(round(perc_adj, digits = 1), "%"), 
    vjust = -0.5)) +
...
```{r, include = FALSE}
tiff(
  file   = "output/Fig 4a–b (habitat type frequency).tiff", 
  width  = 2100, 
  height = 1000, 
  units  = "px", 
  res    = 300)
...
lidar_os_plot <- ggplot(
  ...
  scale_x_discrete(   labels = wrap_format(3)) + #library(scales) +
  xlab("") + 
  ylab("Overstory cover (%)")

print(lidar_os_plot)
```

```{r, fig.align = "center", fig.width = 9, fig.height = 3}
lidar_us_plot <- ggplot(
  data = lidar_us, 
  aes(x = type, 
      y = raster_units)) +
  ...
  xlab("") + 
  ylab("Understory cover (%)")

print(lidar_us_plot)
```

```{r}
lidar_as_plot <- ggplot(
  data = lidar_as, 
  aes(x = type, 
      y = raster_units)) +
...
```{r, include = FALSE}
# Combine plots a–c
lidar_avail_plots_abc <- ggarrange(
  lidar_os_plot, 
  lidar_us_plot, 
  lidar_as_plot, 
...
# Combine plots d–e
lidar_period_plots_de <- ggarrange(
  lidar_us_period_noc, 
  lidar_as_period_noc,
...
# Combine plots f–g
lidar_period_plots_fg <- ggarrange(
  lidar_os_period_diu, 
  lidar_as_period_diu,
...
#Combine plots a–g
lidar_plots_ag <- ggarrange(
  lidar_avail_plots_abc, 
  lidar_period_plots_de, 
  lidar_period_plots_fg, 
  nrow = 3)

tiff(
  file = "output/Fig 5a–g (habitat attribute by period).tiff", 
  height = 2750, 
  width = 2500, 
  units = "px", 
  res = 300)
...
```{r, eval = FALSE}
data <- read.csv("output/coordinates curated.csv") %>% 
  mutate(date_time_12hr = as.POSIXct(strptime(
    date_time_12hr, 
    format = "%Y-%m-%d %H:%M",
    tz = "Australia/Sydney")))

data_ltraj <- as.ltraj(
  xy = data[,c("easting", "northing")], 
  date = data$date_time_12hr, id = data$name, 
  proj4string = CRS(utm)) #plot(data_ltraj)

list <- data.frame(
  (rep(c(1:15), each = 15)), #change to unique(data$name)
  (rep(c(1:15), times = 15))) %>%
  rename(C1 = 1, C2 = 2) %>%
...
data_ltraj <- as.ltraj(
  xy = baseline[,c("easting", "northing")], 
  date = baseline$date_time_12hr, 
  id = baseline$name, 
  proj4string = CRS(utm)) #plot(data_ltraj)

list <- data.frame(
  (rep(c(1:7), each = 7)), #change to unique(data$name)
  (rep(c(1:7), times = 7))) %>%
  rename(C1 = 1, C2 = 2) %>%
  subset(as.character(C1) ! = as.character(C2))
...
data_ltraj <- as.ltraj(
  xy = release[,c("easting", "northing")], 
  date = release$date_time_12hr, 
  id = release$name, 
  proj4string = CRS(utm)) #plot(data_ltraj)

list <- data.frame(
  (rep(c(1:14), each = 14)), #change to unique(data$name)
  (rep(c(1:14), times = 14))) %>%
  rename(C1 = 1, C2 = 2) %>%
  subset(as.character(C1) ! = as.character(C2))
...
data_ltraj <- as.ltraj(
  xy = settlement[,c("easting", "northing")], 
  date = settlement$date_time_12hr, id = settlement$name, 
  proj4string = CRS(utm)) #plot(data_ltraj)

list <- data.frame(
  (rep(c(1:10), each = 10)), #change to unique(data$name)
  (rep(c(1:10), times = 10))) %>%
  rename(C1 = 1, C2 = 2) %>%
  subset(as.character(C1) ! = as.character(C2))
...
coden <- read_excel(raw_data, sheet = "dens") %>% 
  ...
  subset(night >= 3 & night <= 52) %>% 
  subset(ifelse(cohort == "Resident", night <= 32, night >= 22)) %>%
  select(name, morph, cohort, period, coden, eqd_code) %>%
...
sharing_plot <- sharing %>%
  ...
  mutate(
    probability = (`Den shared`/sum(`Den shared`, Alone)), 
    cohort = factor(cohort, levels = c("Resident", "Reinforcer")))
```
...
tiff(
  file = "output/Fig 6a–c (associations).tiff", 
  width = 3000, 
  height = 1000, 
  units = "px",
  res = 300)
...
mod <- lmer(
  distance_night ~ (cohort*period) + morph + 
    (1|name) + (1|day) + (1|precip) + (1|min_temp) + (1|moon), 
  data = model_data)
anova(mod) #F-value
car::Anova(mod) #chisq + p-value
...
# Nocturnal activity
mod <- lmer(
  percentage ~ cohort*period + morph + (1|name), 
  data = woodland)
anova(mod) #F-value
car::Anova(mod) #chisq + p-value, 
summary(mod) #random effects
...
mod <- lmer(
  overstory ~ cohort*period + morph + (1|name), 
  data = noc_lidar)
anova(mod) #F-value
car::Anova(mod) #chisq + p-value, 
summary(mod) #random effects
...
mod <- lmer(
  understory ~ cohort*period + morph + (1|name), 
  data = noc_lidar)
anova(mod) #F-value
car::Anova(mod) #chisq + p-value, 
summary(mod) #random effects
...
mod <- lmer(
  aspect ~ cohort*period + morph + (1|name), 
  data = noc_lidar)
anova(mod) #F-value
car::Anova(mod) #chisq + p-value, 
summary(mod) #random effects
...
mod <- lmer(understory ~ cohort*period + morph + (1|name), 
            data = diu_lidar)
anova(mod) #F-value
car::Anova(mod) #chisq + p-value, 
summary(mod) #random effects

mod <- lmer(aspect ~ cohort*period + morph + (1|name), 
            data = diu_lidar)
anova(mod) #F-value
car::Anova(mod) #chisq + p-value, 
summary(mod) #random effects
...
mod <- glm(
  bin ~ cohort + period + morph, data = coden, 
  family = binomial(link = "logit"))
anova(mod) #F-value
car::Anova(mod) #chisq + p-value
```
