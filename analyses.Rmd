...
pacman::p_load(adehabitatHR, adehabitatLT, AICcmodavg, brglm,
               corrplot, data.table, dplyr, ggplot2, ggpmisc,
               ggpubr, ggspatial, grid, gridExtra, gtools,
               janitor, lme4, lubridate, MuMIn, plotrix, 
               raster, readxl, rgdal, RODBC, scales, sf, 
               sp, tidyr, tidyverse, viridis, wildlifeDI)
```
...
# Plot coordinates 
ggplot() + 
  geom_path(mfws, mapping=aes(x=long, y=lat, group=group), col="grey30") +
...
duration <- ggplot(coords_annotated, 
                   mapping=aes(x=fence, y=duration, fill=fence, col=fence)) + 
  geom_violin(col="grey30", alpha=0.7, lwd=0.6) +
  geom_boxplot(col="grey30", fill="grey30", width=0.02, lwd=0.5) +
  stat_summary(fun="median", geom="point", col="grey30", fill="white", 
...
hdop <- ggplot(coords_annotated, mapping=aes(x=fence, y=dop, fill=fence)) +
  geom_violin(col="grey30", alpha=0.7, lwd=0.6) +
  geom_boxplot(col="grey30", fill="grey30", width=0.02, lwd=0.5) +
  stat_summary(fun="median", geom="point", col="grey30", fill="white", 
               shape=21, size=1.5, stroke=1) +
...
satellites <- ggplot(coords_annotated, 
                     mapping=aes(x=fence, y=satellites, fill=fence)) +
  geom_violin(col="grey30", alpha=0.7, lwd=0.6) +
  geom_boxplot(col="grey30", fill="grey30", width=0.02, lwd=0.5) +
  stat_summary(fun="median", geom="point", col="grey30", fill="white", 
               shape=21, size=1.5, stroke=1) +
...
```{r, include=FALSE}
ggsave(filename="figures/Fig 0 (data curation).tiff", 
...
write.csv(coords_f, "processed data/coordinates curated.csv")
```
...
```{r}
data <- read.csv("processed data/coordinates curated.csv")
...
dist_plot <- ggplot(data=distance_night, 
  ...
  geom_violin(aes(fill=period), col="grey30", alpha=0.7, lwd=0.6) +
  geom_boxplot(col="grey30", fill="grey30", width=0.04, lwd=0.6) +
  stat_summary(fun="median", geom="point", col="grey30", fill="white", 
               shape=21, size=1.5, stroke=1) +
  #annotate(geom="text", x=0.7, y=6, label="a)", fontface="bold", size=6) +
  theme(panel.grid.major=element_blank(),
        panel.grid.minor=element_blank(),
        panel.background=element_rect(fill="white"),
...
write.csv(ranges, "processed data/raw ranges per day.csv")
```

```{r}
ranges_day <- read.csv("processed data/raw ranges per day.csv") %>% #format time
...
periods <- read.csv("processed data/coordinates curated.csv") %>%
  group_by(cohort, period, morph, name) %>%
  summarise(fixes=length(name))
study <- read.csv("processed data/coordinates curated.csv") %>%
...
write.csv(ranges_data,"processed data/ranges per day.csv")
```
...
write.csv(ranges, "processed data/ranges per period.csv")
```
...
```{r}
ranges_model <- read.csv("processed data/ranges per period.csv") %>% 
  subset(period!="Study") %>%
  mutate(cohort=factor(cohort, levels=c("Resident","Reinforcer")))
...
ranges_data <- read.csv("processed data/ranges per day.csv") %>%
...
```{r, include=FALSE}
tiff(file="figures/Fig 3 (ranges map).tiff", 
...
write.csv(overlap, "processed data/overlap per individual.csv")
```

### Models and summaries

```{r}
overlap <- read.csv("processed data/overlap per individual.csv") %>%
    mutate(cohort=factor(cohort, levels=c("Resident","Reinforcer")))
...
home_period <- ggplot(data=ranges_model, 
  ...
  stat_summary(position=pos, aes(col=cohort), fun="median", geom="point", 
               fill="white", shape=21, size=1.5, stroke=1) +
  #annotate(geom="text", x=0.7, y=600, label="b)", fontface="bold", size=6) +
  theme(panel.grid.major=element_blank(),
        panel.grid.minor=element_blank(),
        panel.background=element_rect(fill="white"),
        plot.margin=unit(c(0.1, 0, 0, 0.2), "cm"),
...
home_time <- ggplot(data=ranges_data, aes(x=night_12hr, 
  ...
  geom_smooth(method='lm', aes(fill=cohort, col=cohort), lwd=0.8, alpha=0.5) +
  #annotate(geom="text", x=50, y=300, label="c)", fontface="bold", size=6) +
  theme(panel.grid.major=element_blank(),
        panel.grid.minor=element_blank(),
        panel.background=element_rect(fill="white"),
        plot.margin=unit(c(0.1, 0, 0, 0.2), "cm"),
...
home_overlap_plot <- ggplot(overlap, aes(x=cohort, 
  ...
  stat_summary(position=pos, aes(col=cohort), fun="median", geom="point", 
               fill="white", shape=21, size=1.5, stroke=1) +
  #annotate(geom="text", x=0.7, y=250, label="d)", fontface="bold", size=6) + 
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
        panel.background = element_rect(fill="white"),
        plot.margin = unit(c(0.1, 0, 0, 0.2), "cm"),
...
tiff(file="figures/Fig 2a–d (distance and ranges).tiff", 
     height=1550, width=2100, units="px", res=300)
dist_ranges_plots
dev.off()
```
...
```{r, include=FALSE}
tiff(file="figures/Fig 1a–b (habitat type and map).tiff", 
...
```{r}
#data for plots
noc <- read.csv("processed data/coordinates curated.csv") %>%
  group_by(mfws_habitat) %>% count() %>% 
...
noc_period <- read.csv("processed data/coordinates curated.csv") %>%
  group_by(cohort, period, morph, name, mfws_habitat) %>%
...
# Read in raster data
overstory <- raster("rasters/mfgo_overstory.tif")
understory <- raster("rasters/mfgo_understory.tif")
aspect <- raster("rasters/mfgo_aspect.tif")
...
```{r, include=FALSE}
tiff(file="figures/Fig 4a–b (habitat type frequency).tiff", 
...
lidar_os_plot <- ggplot(data=lidar_os, aes(x=type, y=raster_units)) +
  ...
  geom_boxplot(position=pos, aes(col=type), 
               fill="grey30", width=0.03, lwd=0.6) +
  stat_summary(position=pos, aes(col=type), fun="median", geom="point", 
               fill="white", shape=21, size=1.5, stroke=1) +
  #annotate(geom="text", x=0.7, y=72, label="a)", fontface="bold", size=6) +
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
        panel.background = element_rect(fill="white"),
        axis.line = element_line(colour="black"),
...
lidar_us_plot <- ggplot(data=lidar_us, 
  ...
  stat_summary(position=pos, aes(col=type), fun="median", geom="point", 
               fill="white", shape=21, size=1.5, stroke=1) +
  #annotate(geom="text", x=0.7, y=39, label="b)", fontface="bold", size=6) +
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
        panel.background = element_rect(fill="white"),
        axis.line = element_line(colour="black"),
...
lidar_as_plot <- ggplot(data=lidar_as, aes(x=type, y=raster_units)) +
  ...
  stat_summary(position=pos, aes(col=type), fun="median", geom="point", 
               fill="white", shape=21, size=1.5, stroke=1) +
  #annotate(geom="text", x=0.7, y=355, label="c)", fontface="bold", size=6) +
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
        panel.background = element_rect(fill="white"),
        axis.line = element_line(colour="black"),
...
lidar_us_period_noc <- ggplot(data=noc_lidar, 
  ...
  geom_boxplot(position=pos, aes(col=period), 
               fill="grey30", width=0.03, lwd=0.6) +
  stat_summary(position=pos, aes(col=period), fun="median", geom="point", 
               fill="white", shape=21, size=1.5, stroke=1) +
  #annotate(geom="text", x=0.7, y=30, label="d)", fontface="bold", size=6) +
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
        panel.background = element_rect(fill="white"),
        axis.line = element_line(colour="black"),
  ...
  scale_x_discrete(labels = wrap_format(3)) + #library(scales) +
  scale_fill_manual(values = viridis(3, begin=0.9, end=0.1)) +
  xlab("Study period") + ylab("Understory layer cover (%)")
...
lidar_as_period_noc <- ggplot(data=noc_lidar, 
  ...
  stat_summary(position=pos, aes(col=period), fun="median", geom="point", 
               fill="white", shape=21, size=1.5, stroke=1) +
  #annotate(geom="text", x=0.7, y=350, label="e)", fontface="bold", size=6) +
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
        panel.background = element_rect(fill="white"),
        axis.line = element_line(colour="black"),
  ...
  scale_x_discrete(labels = wrap_format(3)) + #library(scales) +
  scale_fill_manual(values = viridis(3, begin=0.9, end=0.1)) +
  xlab("Study period") + ylab("Aspect (degrees)")
...
lidar_os_period_diu <- ggplot(data=diu_lidar, 
  ...
  stat_summary(position=pos, aes(col=period), fun="median", geom="point", 
               fill="white", shape=21, size=1.5, stroke=1) +
  #annotate(geom="text", x=0.7, y=55, label="f)", fontface="bold", size=6) +
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
        panel.background = element_rect(fill="white"),
        axis.line = element_line(colour="black"),
  ...
  scale_x_discrete(labels = wrap_format(3)) + #library(scales) +
  scale_fill_manual(values = viridis(3, begin=0.9, end=0.1)) +
  xlab("Study period") + ylab("Overstory layer cover (%)")
...
lidar_as_period_diu <- ggplot(data=diu_lidar, 
  ...
  stat_summary(position=pos, aes(col=period), fun="median", geom="point", 
               fill="white", shape=21, size=1.5, stroke=1) +
  #annotate(geom="text", x=0.7, y=350, label="g)", fontface="bold", size=6) +
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
        panel.background = element_rect(fill="white"),
        axis.line = element_line(colour="black"),
...
tiff(file="figures/Fig 5a–g (habitat attribute by period).tiff", 
...
write.csv(interact, "processed data/interaction coefficients.csv")
```
...
```{r}
interact <- read.csv("processed data/interaction coefficients.csv") %>%
  mutate(name=quoll1) %>%
  na.omit() %>%
  merge(lookup, by=c("name", "period")) %>%
  mutate(cohort=factor(cohort, levels=c("Resident", "Reinforcer")),
...
pear_period_plot <- ggplot(data=interact, 
  ...
  geom_boxplot(position=pos, aes(col=morph), 
               fill="grey30", width=0.04, lwd=0.6) +
  stat_summary(position=pos, aes(col=morph), fun="median", geom="point", 
               fill="white", shape=21, size=1.5, stroke=1) +
  #annotate(geom="text", x=0.7, y=0.75, label="a)", fontface="bold", size=6) +
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
        panel.background = element_rect(fill="white"),
...
soc_period_plot <- ggplot(data=interact, 
  ...
  stat_summary(position=pos, aes(col=period), fun="median", geom="point", 
               fill="white", shape=21, size=1.5, stroke=1) +
  #annotate(geom="text", x=0.7, y=0.75, label="b)", fontface="bold", size=6) + 
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
        panel.background = element_rect(fill="white"),
        plot.margin = unit(c(0.1, 0, 0, 0.1), "cm"),
...
coden_plot <- ggplot(data=sharing_plot, 
  ...
  stat_summary(position=pos, aes(col=cohort), fun="median", geom="point", 
               fill="white", shape=21, size=1.5, stroke=1) +
  #annotate(geom="text", x=0.7, y=0.95, label="c)", fontface="bold", size=6) +
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
        panel.background = element_rect(fill="white"),
        plot.margin = unit(c(0.1, 0, 0, 0), "cm"),
...
```{r, include=FALSE}
interact_plot <- ggarrange(pear_period_plot, soc_period_plot, 
                           coden_plot, ncol=3, nrow=1, label.x=0.89, 
                           widths=c(3,3,3), label.y=0.99, hjust=-0.1, vjust=1.5, 
                           font.label=list(size=10, face="bold", color="black"))
print(interact_plot)

tiff(file="figures/Fig 6a–c (associations).tiff", 
...
quoll_animation <- read.csv("processed data/coordinates curated.csv") %>%
