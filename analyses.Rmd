...
pacman::p_load(adehabitatHR, adehabitatLT, corrplot, data.table, dplyr,
               ggplot2, ggpmisc, ggpubr, ggspatial, grid, gridExtra, gtools, janitor,
               lme4, lubridate, MuMIn, plotrix, rstudioapi, raster, readxl,
               rgdal, RODBC, scales, sf, sp, tidyr, tidyverse, viridis,
               wildlifeDI)
```
...
hr_map <- ggplot(hr_lookup) + 
  ...
  annotation_scale(style="ticks", tick_height=0.5,
                   width_hint=0.3, pad_x=unit(4.2, "cm"),
                   pad_y=unit(0.2, "cm")) +
  theme(panel.grid.major = element_blank(), 
        panel.grid.minor = element_blank(),
        panel.border = element_blank(),
  ...
  scale_colour_viridis(discrete=T, option="D", begin=1, end=0) +   facet_wrap(~cohort) +
  xlab("") + ylab("")
print(hr_map)
```

```{r, include=FALSE}
jpeg(file="figures/home range map by cohort.jpeg", 
     width=3000, height=1500, units="px", res=500)
hr_map
dev.off()
```
...
```{r, include=FALSE}
jpeg(file="figures/core range map by cohort.jpeg", 
     width=3000, height=1500, units="px", res=500)
cr_map
dev.off()
```

Generate version of map where core ranges has no facet strip.
...
ranges_map <- ggarrange(hr_map, cr_map, 
                         ncol=1, nrow=2, widths=c(3,3))
ranges_map

jpeg(file="figures/ranges map by cohort.jpeg", 
     width=3000, height=3000, units="px", res=500)
ranges_map
dev.off()
```
