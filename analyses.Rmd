...
pacman::p_load(adehabitatHR, adehabitatLT, brglm, corrplot, data.table,
               dplyr, ggplot2, ggpmisc, ggpubr, ggspatial, grid,
               gridExtra, gtools, janitor, lme4, lubridate, MuMIn,
               plotrix, raster, readxl, rgdal, RODBC, scales, sf, 
...
round(coef(summary(glm(fence ~ duration + temp + 
                       dop + satellites, 
...
temperature <- ggplot() + 
  geom_boxplot(coords_annotated, mapping=aes(x=fence, y=temp, fill=fence), 
               col="grey15", alpha=0.8) +
  theme(legend.position="none", 
        axis.line=element_line(colour="black"),
        panel.background=element_rect(fill="white")) +
...
```{r, include=FALSE}
ggsave(filename="figures/data curation boxplots.jpeg", 
...
```{r, include=FALSE}
jpeg(file="figures/ranges map by cohort and type.jpeg", 
     width=3000, height=2650, units="px", res=500)
hr_cr_map
dev.off()
```
...
home_period <- ggplot(data=ranges_model, 
  ...
  annotate(geom="text", x=0.7, y=600, label="b)", 
...
home_time <- ggplot(data=ranges_data, aes(x=night_12hr, 
  ...
  geom_smooth(method='lm', aes(fill=cohort, col=cohort), 
              alpha=0.5) +
  annotate(geom="text", x=50, y=300, label="c)", 
           fontface="bold", size=6) +
  theme(panel.grid.major=element_blank(),
        panel.grid.minor=element_blank(),
        panel.background=element_rect(fill="white"),
...
home_overlap_plot <- ggplot(overlap, aes(x=cohort, 
  ...
  geom_boxplot(aes(fill=cohort), col="grey15", alpha=0.8) +
  annotate(geom="text", x=0.7, y=250, label="d)", 
           fontface="bold", size=6) + 
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
        panel.background = element_rect(fill="white"),
...
jpeg(file="figures/distance and ranges by fixed effects.jpeg", 
     width=3500, height=2500, units="px", res=500)
dist_ranges_plots
dev.off()
```
...
```{r, include=FALSE}
jpeg(file="figures/nvis habitat type barplot and map.jpeg", 
...
noc_nvis_plot <- ggplot(noc_plot, 
  ...
  annotate(geom="text", x=1, y=1500, label="a)", 
           fontface="bold", size=6) +
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
        panel.background = element_blank(),
...
```{r, include=FALSE}
jpeg(file="figures/nvis habitat type frequency.jpeg", 
...
lidar_us_period_noc <- ggplot(data=noc_lidar, 
  ...
  geom_boxplot(aes(fill=period), col="grey15", alpha=0.8) +
  annotate(geom="text", x=0.7, y=30, label="d)", 
           fontface="bold", size=6) +
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
        panel.background = element_rect(fill="white"),
...
lidar_os_period_diu <- ggplot(data=diu_lidar, 
  ...
  geom_boxplot(aes(fill=period), col="grey15", alpha=0.8) +
  annotate(geom="text", x=0.7, y=55, label="f)", 
           fontface="bold", size=6) +
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
        panel.background = element_rect(fill="white"),
...
```{r, include=FALSE}
lidar_avail_plots <- ggarrange(lidar_os_plot, lidar_us_plot, 
...
jpeg(file="figures/lidar habitat use by availability.jpeg", 
     width=4500, height=1500, units="px", res=500)
print(lidar_avail_plots)
dev.off()

lidar_period_plots <- ggarrange(lidar_us_period_noc, lidar_as_period_noc,
                        lidar_os_period_diu, lidar_as_period_diu,
                        hjust=-0.1, vjust=1.5, nrow=2, ncol=2,
                        widths=c(2,2,2,2), 
                        label.x=0.89, label.y=0.99, 
                        font.label=list(size=10, face="bold",
                                        color="black"))

jpeg(file="figures/lidar habitat use by period.jpeg", 
     width=4000, height=3000, units="px", res=500)
print(lidar_period_plots)
dev.off()
```
...
pear_period_plot <- ggplot(data=interact, 
  ...
  annotate(geom="text", x=0.7, y=0.75, label="a)", 
           fontface="bold", size=6) +
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
        panel.background = element_rect(fill="white"),
...
soc_period_plot <- ggplot(data=interact, 
                          aes(x=period, y=as.numeric(cr))) +
  geom_boxplot(aes(fill=period), col="grey15", alpha=0.8) +
  annotate(geom="text", x=0.7, y=0.75, label="b)", 
           fontface="bold", size=6) + 
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
        panel.background = element_rect(fill="white"),
...
coden_plot <- ggplot(data=sharing_plot, 
  ...
  geom_boxplot(aes(fill=cohort), col="grey15", alpha=0.8) +
  annotate(geom="text", x=0.7, y=0.95, label="c)", 
           fontface="bold", size=6) +
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
        panel.background = element_rect(fill="white"),
...
interact_plot <- ggarrange(pear_period_plot, soc_period_plot, 
                           coden_plot, ncol=3, nrow=1, 
                           label.x=0.89, widths=c(3,3,3),
                           label.y=0.99, hjust=-0.1, vjust=1.5, 
                           font.label=list(size=10, face="bold", 
                                           color="black"))
print(interact_plot)

jpeg(file="figures/associations by fixed effects.jpeg", 
     width=5500, height=1750, units="px", res=500)
print(interact_plot)
dev.off()
```
